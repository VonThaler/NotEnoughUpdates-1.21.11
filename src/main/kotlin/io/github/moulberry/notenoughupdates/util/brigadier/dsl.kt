...
fun <T : FabricClientCommandSource, C : CommandContext<T>> C.reply(component: Text) {
    source.sendFeedback(Text.literal("§e[NEU] ").append(component))
}

fun <T : FabricClientCommandSource, C : CommandContext<T>> C.reply(text: String, block: Text.() -> Unit = {}) {
    source.sendFeedback(Text.literal(text.split("\n").joinToString("\n") { "§e[NEU] $it" }))
}
